...
public class LoginServlet extends HttpServlet{
	...
	private void doService(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(username.equalsIgnoreCase("Aadvik") && password.equalsIgnoreCase("Aadvik")){
			authentication =true;
		}
		...
		if(authentication){
			...
			session.invalidate();//if it returns the old session...
			session = req.getSession();
			session.setAttribute("username", username);
			RequestDispatcher redispatcher =req.getRequestDispatcher("productpage");
